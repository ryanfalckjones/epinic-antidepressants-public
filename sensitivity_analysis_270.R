...
# 270 days before ICU-admission
no_prior_antidepressants <-
  antidepressant_exposure %>%
  filter(date == sir_adm_date) %>%
  mutate(prior_ad = case_when(is.na(last_exp) ~ FALSE,
                              last_exp < sir_adm_date - days(270) ~ FALSE,
                              last_exp >= sir_adm_date - days(270) ~ TRUE)) %>%
  filter(!prior_ad) %>%
  select(CONT_ICU_ID) %>%
  pull()
...
# Save RDS 
readr::write_rds(df$epinic %>% select(-CONT_ICU_ID), "data/df_epinic_270.rds")
readr::write_rds(df$epinic, "data/df_epinic_incl_id_270.rds")
readr::write_rds(antidepressant_graph, "data/df_epinic_ad_graph_270.rds")