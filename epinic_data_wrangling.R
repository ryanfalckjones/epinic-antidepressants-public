...
allopurinol_dispensed <- 
  "SELECT LopNr, ATC, EDATUM, FDATUM, subnamn
 FROM LMED
 WHERE ATC LIKE '%M04AA01%'
  AND LopNr IN (SELECT LopNr FROM pts);" %>%
  sqlQuery() %>% 
  as_tibble() %>%
  group_by(LopNr) %>%
...
# Allopurinol graph
allopurinol_exposure <- 
  observation_dates %>%
  left_join(allopurinol_dispensed) %>%
  arrange(date, .by_group = TRUE) %>%
  rename(last_exp = EDATUM) %>%
  fill(last_exp, .direction = 'down') %>%
  mutate(exposure = data.table::fifelse(!is.na(last_exp) & date <= last_exp + days(90), TRUE, FALSE),
...
allopurinol_graph <- allopurinol_exposure %>% 
  group_by(relative_time, exposure) %>%
  summarise(count = n()) %>%
  mutate(per = 100 * count/sum(count)) %>%
  filter(exposure,
...
thyroxin_graph <- thyroxin_exposure %>% 
  group_by(relative_time, exposure) %>%
  summarise(count = n()) %>%
  mutate(per = 100 * count/sum(count)) %>%
  filter(exposure,
...
# 180 days before ICU-admission
no_prior_antidepressants <-
  antidepressant_exposure %>%
  filter(date == sir_adm_date) %>%
  mutate(prior_ad = case_when(is.na(last_exp) ~ FALSE,
                              last_exp < sir_adm_date - days(180) ~ FALSE,
                              last_exp >= sir_adm_date - days(180) ~ TRUE)) %>%
  filter(!prior_ad) %>%
  select(CONT_ICU_ID) %>%
  pull()
...
# Save RDS 
readr::write_rds(df$epinic %>% select(-CONT_ICU_ID), "data/df_epinic.rds")
readr::write_rds(df$epinic, "data/df_epinic_incl_id.rds")
readr::write_rds(antidepressant_graph, "data/df_epinic_ad_graph.rds")
readr::write_rds(statin_graph, "data/df_epinic_statin_graph.rds")
readr::write_rds(allopurinol_graph, "data/df_epinic_allopurinol_graph.rds")
readr::write_rds(thyroxin_graph, "data/df_epinic_thyroxin_graph.rds")

# Perform multiple imputation
source("source/multiple_imputation.R")